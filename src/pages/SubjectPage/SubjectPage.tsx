import React, {
  useRef, useState, TouchEvent, CSSProperties,
} from 'react';
...
function SubjectPage() {
  ...
  const trayOffsetStyle = () => {
    if (isAssignmentListTrayDragUp) {
      return styleToDragUp;
    }
    if (assignmentListTrayDragReset) {
      return dragReset;
    }
    return onDragging;
  };
  ...
  return (
    <div className={styles.subjectPage}>
      ...
      <div
        ref={assignmentListTray}
        // onTouchStart={handleSubjectListWrapperOnTocuhStart}
        onTouchStart={(event) => handleSubjectListWrapperOnToucStart(event)}
        onTouchMove={(event) => handleSubjectListWrapperOnTouchMove(event)}
        onTouchEnd={handleSubjectListWrapperOnTouchEnd}
        className={styles.assignmentListWrapper}
        style={trayOffsetStyle()}
      >
        <AssignmentList />
      </div>
    </div>
